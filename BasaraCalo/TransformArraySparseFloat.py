...
def make_reduced(f):
    ...
    reduced[..., 2] = np.minimum(
        np.log(np.maximum(f["Particles"][..., features.index("Pt")], 1.001)) / 5.0, 10
    )
    reduced[..., 3] = np.argmax(f["Particles"][..., 13:], axis=-1)

    h_reduced = np.zeros((pf.shape[0], 1, 4))
    h_reduced[..., 0, 2] = np.minimum(
        np.maximum(np.log(f["HLF"][..., 1]) / 5.0, 0.001), 10
    )  # MET
...
def convert_sample(inFileName, limit=None):
    ...
    outFileName = "ArrayInputs_900/f" + inFileName.rsplit("/", 1)[-1]
    if limit:
