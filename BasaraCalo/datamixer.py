...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Data mixer", formatter_class=argparse.RawTextHelpFormatter
    )
    parser.add_argument(
        "--config", type=str, required=True, help="custom config file for mixing data"
    )
...
def melanger3(
    chemin, liste_chemins, taille=1000, nb_max_boucles=-5
):  # chemin(str) is the path to the directory where the file with the mixed datas
    ...
    liste_fichiers_h5 = [
        os.listdir(liste_chemins[0]),
        os.listdir(liste_chemins[1]),
        os.listdir(liste_chemins[2]),
    ]

    liste_nb = [
        len(liste_fichiers_h5[0]),
        len(liste_fichiers_h5[1]),
        len(liste_fichiers_h5[2]),
    ]
    ...
    while (
        liste_counters[0] <= liste_nb[0]
        or liste_counters[1] <= liste_nb[1]
        or liste_counters[2] <= liste_nb[2]
    ):  # if there always are files...

        liste_iterateurs[2] = liste_iterateurs[2] + 1
        print(liste_iterateurs[2])

        if (
            liste_iterateurs[0] == taille or liste_iterateurs[2] == nb_max_boucles
        ):  # the exit files consist in many little files

            donnees_melangees = h5py.File(
                chemin + "//" + "mixed_data_" + str(liste_iterateurs[1]) + ".h5", "w"
            )  # a h5py file is created
            # with mixed datas
            ...
            labelse = donnees_melangees.create_dataset(
                name="Labels", data=labels, dtype="f8"
            )
            particlese = donnees_melangees.create_dataset(
                name="Particles", data=particles, dtype="f8"
            )
            ...
            if liste_au_debut[choix]:  # able the initialisation
                ...
                liste_typefichier[choix] = h5py.File(
                    liste_chemins[choix]
                    + "//"
                    + liste_fichiers_h5[choix][liste_counters[choix]],
                    "r",
                )
                ...
                if liste_minicounters[choix] < len(
                    liste_typefichier[choix]["Particles"]
                ):  # ... but not at the end

                    hLF.append(
                        list(liste_typefichier[choix]["HLF"][liste_minicounters[choix]])
                    )
                    labels.append(liste_labels[choix])
                    particles.append(
                        list(
                            liste_typefichier[choix]["Particles"][
                                liste_minicounters[choix]
                            ]
                        )
                    )
                    ...
                    if (
                        liste_counters[choix] < liste_nb[choix]
                    ):  # ...if there are always files
...
if __name__ == "__main__":
    ...
    melanger3(
        config.path.outpath,
        [config.path.inpath_qcd, config.path.inpath_ttbar, config.path.inpath_w],
        config.algorithm.nevent,
        config.algorithm.maxloop,
    )
